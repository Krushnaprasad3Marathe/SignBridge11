...
import { Camera, Volume2, Trash2, Delete, Play, Square, Info, History, ArrowLeft } from 'lucide-react';
...
interface SignBridgeProps {
  onBack: () => void;
}

export default function SignBridge({ onBack }: SignBridgeProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 p-4 md:p-8">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-slate-500 font-medium">Core ASL Translation Module</p>
            </div>
          </div>
                  ...
                  >
                    ...
                    <div className="flex flex-col">
                      ...
                      <div className="w-32 h-2 bg-white/20 rounded-full mt-1 overflow-hidden">
                        <motion.div className="h-full bg-emerald-400" animate={{ width: `${confidence}%` }} />
                      </div>
                    </div>
                  </motion.div>
                )}
          ...
          <section className="space-y-6">
            <div className="bg-white rounded-2xl p-6 shadow-xl border border-slate-200 h-full flex flex-col">
              ...
              <div className="flex-1 bg-slate-50 rounded-xl p-6 mb-6 border-2 border-dashed border-slate-200 min-h-[200px] relative group">
                ...
                {word && (
                  <button onClick={() => speak(word)} className="absolute top-2 right-2 p-2 bg-indigo-100 text-indigo-600 rounded-lg hover:bg-indigo-600 hover:text-white transition-colors">
              ...
              <AnimatePresence>
                {aiSuggestion && aiSuggestion !== word && (
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="bg-emerald-50 border border-emerald-100 rounded-xl p-4 space-y-2">
                    ...
                    <div className="flex items-center justify-between">
                      <p className="text-2xl font-black text-emerald-900">{aiSuggestion}</p>
                      <button onClick={() => { setWord(aiSuggestion); setAiSuggestion(""); }} className="px-3 py-1 bg-emerald-600 text-white text-xs font-bold rounded-lg hover:bg-emerald-700 transition-all">Accept</button>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
              <div className="grid grid-cols-2 gap-3 mb-6">
                <button onClick={addSpace} className="flex items-center justify-center gap-2 px-4 py-3 bg-slate-100 hover:bg-slate-200 rounded-xl font-semibold text-slate-700">Space</button>
                <button onClick={backspace} disabled={!word} className="flex items-center justify-center gap-2 px-4 py-3 bg-slate-100 hover:bg-slate-200 disabled:opacity-50 rounded-xl font-semibold text-slate-700"><Delete className="w-4 h-4" />Back</button>
                <button onClick={() => speak(word)} disabled={!word} className="col-span-2 flex items-center justify-center gap-2 px-4 py-4 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white rounded-xl font-bold shadow-lg shadow-indigo-100 transition-all"><Volume2 className="w-5 h-5" />Speak Word</button>
                <button onClick={clearWord} disabled={!word} className="col-span-2 flex items-center justify-center gap-2 px-4 py-3 border-2 border-slate-100 hover:bg-rose-50 hover:text-rose-600 hover:border-rose-100 disabled:opacity-50 rounded-xl font-semibold text-slate-500 transition-all"><Trash2 className="w-4 h-4" />Clear & Archive</button>
              </div>
              <div className="space-y-3">
                <h3 className="text-xs font-bold uppercase tracking-widest text-slate-400 flex items-center gap-2"><History className="w-3 h-3" />Recent Words</h3>
                <div className="space-y-2">
                  {history.map((h, i) => (
                    <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="text-sm font-medium text-slate-600 bg-slate-50 px-3 py-2 rounded-lg flex justify-between items-center">
                      <span>{h}</span>
                      <button onClick={() => speak(h)} className="text-slate-400 hover:text-indigo-600"><Volume2 className="w-3 h-3" /></button>
                    </motion.div>
                  ))}
        ...
        <footer className="text-center text-slate-400 text-sm py-4">Sign Bridge Module • Privacy First: No video data leaves your device.</footer>
      </div>
    </div>
  );
}
